...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateUser = useCallback(async (updatedUser: AuthUser) => {
    await SecureStore.setItemAsync("trendlify_user", JSON.stringify(updatedUser));
    setUser(updatedUser);
  }, []);

  return (
    <AuthContext value={{ token, user, isLoggedIn: !!token, login, logout, updateUser }}>
      {children}
    </AuthContext>
  );
}
